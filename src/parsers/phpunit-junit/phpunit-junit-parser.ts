...
import {JunitReport, SingleSuiteReport, TestCase, TestSuite} from './phpunit-junit-types'
...
/**
 * Parses JUnit XML report generated by PHPUnit.
 *
 * Has been heavily inspired by the `JavaJunitParser` class.
 */
export class PHPUnitJunitParser implements TestParser {
  ...
  private exceptionThrowSource(stackTrace: string): {filePath: string; line: number} | undefined {
    ...
    for (const str of lines) {
      const src = this.getPHPUnitSource(str)
      if (src) {
        return src
      }
    }
  ...
  private getPHPUnitSource(stackTrace: string): {filePath: string; line: number} | undefined {
    const match = stackTrace.match(this.re)
    if (match !== null) {
      const [_, filePath, fileName, lineStr] = match
      const cleanedFilePath = filePath.trim() + fileName
      const line = parseInt(lineStr)
      return {
        filePath: this.getRelativePath(cleanedFilePath),
        line
      }
    }
    return undefined
